#!/usr/bin/env bash
#
# Launch Chrome with a remote debugging port on a dedicated profile so the
# Selenium test can ATTACH to it (chrome.debugger.address=127.0.0.1:<port>).
#
# Usage:
#   ./scripts/launch-chrome-debug.sh [port]      # default port 9222
#
# In the Chrome window that opens, sign in to Facebook once by hand (a human
# login is not flagged as a bot). Leave this Chrome running, then in another
# terminal:
...
# debugging port on your default profile. Override the Chrome binary with
